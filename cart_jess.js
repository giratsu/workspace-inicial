...
async function showProducts()
{
    ...
    for (let i = 0; i < inputDeCantidad.length; i++) {
        ...
        inputDeCantidad[i].addEventListener("input", () => { cambioDeprecioFinalSubtotal(idInput, localCart.find((p) => p.id == idInput)) });
...
function linea(product) {
    ...
    <td class="w-25" id="precioFinalSubtotalPorCantidad${product.id}" >${product.currency} ${product.cost}</td>
    `;
...
function cambioDeprecioFinalSubtotal(idInput, producto) {
    let precioFinalSubtotalUnitario = producto.cost;
    ...
    const importe = document.getElementById(`precioFinalSubtotalPorCantidad${id}`);
    
    const cantidad = cantidadNecesaria.value;
    
    importe.innerHTML = `${moneda} ${cantidad * precioFinalSubtotalUnitario}`;
